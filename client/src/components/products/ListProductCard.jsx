...
function ListProductCard({ id,name,brand,price,original,discount,category,image }) {
  return (
    <div>
        <div className='border overflow-hidden w-full h-120 md:h-65 lg:h-60 xl:h-75 lg:h- border-gray-300 flex flex-col md:flex-row rounded-md shadow-lg '>
          <div className='relative group w-full lg:w-[25%] h-full overflow-hidden rounded-l-md'>
            ...
            <img src={image[0]} alt="" className=' block transition-all duration-700 hover:scale-90 rounded-l-md group-hover:hidden w-full h-full' />
            <img src={image[1]?image[1]:image[0]} alt="" className='hidden rounded-t-lg group-hover:block  transition-all duration-700 hover:scale-110 w-full h-full' />
            </Link>
            ...
            <p className='text-xs md:text-[15px] md:w-100 xl:w-150  text-gray-700'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus accusantium error nemo voluptatum vero laborum, unde accusamus sed porro possimus similique sunt, doloribus ipsa? Debitis eligendi sequi a minima rerum?</p>
            ...
            <button className='px-5 border border-primary mt-3 py-2 text-primary rounded text-xs flex items-center text-center justify-center gap-1 cursor-pointer hover:bg-primary hover:text-white'><IoCartOutline className='text-xs md:text-xl'/> ADD TO CART</button>
            </div>
        </div>
    </div>
  )
